...
export const LINK_DOWN_VOCAB: Readonly<Record<LinkDownKey, LinkDownVocab>> = Object.freeze({
  ...
  'err-disabled': down('the port on {device} is error-disabled',
    '{end} was error-disabled by a protection feature (port security, BPDU guard or a mismatched bundle). Remove the cause, then enter "shutdown" and "no shutdown" on it, or wait for automatic recovery if it is configured.'),
