...
import { BANNER_TYPE_ARG, HANDLERS, MSG_INTERFACE_NOT_CONFIGURABLE } from '../grammar/index.js';
...
const iface: CommandHandler = (ctx, args, negate) => {
  ...
  if (existing === undefined) {
    if (!isVirtualInterfaceName(raw)) return { error: MSG_UNKNOWN_INTERFACE };
  ...
  enterMode(ctx, 'config-if', [['interface', target]]);
  return {};
};

/** `ip route <network> <mask> <next-hop|interface>` and its `no` form. */
const ipRoute: CommandHandler = (ctx, args, negate) => {
  ...
  return outcomeOf(ctx.config(['ip', 'route', u32ToIpv4(net), u32ToIpv4(maskValue), nextHop], negate, globalContext()));
};
